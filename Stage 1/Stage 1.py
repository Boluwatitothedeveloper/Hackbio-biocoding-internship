...
def simulate_population_growth(initial_population, growth_rate, generations,):
    ...
    for generation in range(generations):
            hour = generation + 1
            population = initial_population * math.exp(growth_rate * hour) # using math.exp for e^x
            print(f"Generation {generation + 1}: Population = {population:.2f}: hour {hour}")

    return population # return after the loop
...
def time_to_reach_80_percent_carrying_capacity_for_loop(initial_population, growth_rate, carrying_capacity, max_generations):
    ...
    for generation in range(max_generations):
        ...
        if population >= target_population:
            return generation + 1  # Return the generation when target is reached

    return None  # Return None if target is not reached within max_generations
...
distance = hamming_distance("Titobliss01","ladybliss27")
